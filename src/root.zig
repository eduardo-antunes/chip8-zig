...
pub const Instruction = struct {
    ...
    pub fn decode_from(code: u16) Instruction {
        return .{
            .x = @intCast((code & 0xF00) >> 8),
            .y = @intCast((code & 0xF0) >> 8),
            .n = @intCast(code & 0xF),
            .arg = @intCast(code & 0xFF),
            .addr = code & 0xFFF,
            .op = decode(code),
        };
    }
...
const Operation = enum {
    unknow, // sentinel for invalid operation
...
fn decode(fullcode: u16) Operation {
    const op: u4 = @intCast((fullcode & 0xF000) >> 12);
    return switch (op) {
        0x0 => .unknow,
        0x1 => .jump,
        0x2 => .call,
        ...
        0x6 => .load,
        0x7 => .add,
        0x8 => decodeArithmetic(fullcode),
        0x9 => .skrne,
        0xA => .loadi,
        0xB => .jump0,
        0xC => .rand,
        0xD => .draw,
        0xE => decodeKeypad(fullcode),
        0xF => decodeMisc(fullcode),
    };
}

fn decodeArithmetic(fullcode: u16) Operation {
    const op: u4 = @intCast(fullcode & 0xF);
    return switch (op) {
        0x0 => .move,
        0x1 => .or_,
        0x2 => .and_,
...
fn decodeKeypad(fullcode: u16) Operation {
    const op: u8 = @intCast(fullcode & 0xFF);
    return switch (op) {
        0x9E => .skpr,
        0xA1 => .sknpr,
        else => .unknow,
...
fn decodeMisc(fullcode: u16) Operation {
    const op: u8 = @intCast(fullcode & 0xFF);
    return switch (op) {
        0x07 => .moved,
        0x0A => .keyd,
